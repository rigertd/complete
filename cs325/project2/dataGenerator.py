...
# Run brute force only up to A = 40 and first 30 sets of coins
    ...
    with open('brute_results.csv', 'w') as csvoutput:
        ...
        for A in range(1, 41):
            ...
            for coin_set in coin_sets[:30]:
                ...
                row.append(run_time)
            writer.writerow(row)
# Run greedy on all 1000 sets of coins
elif algorithmSelect == 2:
    with open('greedy_results.csv', 'w') as csvoutput:
        writer = csv.writer(csvoutput, delimiter=',', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
        writer.writerow(['', 'Denomination Count'])
        writer.writerow(['A'] + list(range(1, 31)))
        for A in range(1000, MAX_A_VAL + 1000, 1000):
            print('A={}'.format(A))
            row = [A]
            for coin_set in coin_sets:
                ...
                row.append(run_time)
            writer.writerow(row)
# Run dp on only first 30 sets of coins
elif algorithmSelect == 3:
    with open('dp_results.csv', 'w') as csvoutput:
        writer = csv.writer(csvoutput, delimiter=',', quoting=csv.QUOTE_MINIMAL, lineterminator='\n')
        writer.writerow(['', 'Denomination Count'])
        writer.writerow(['A'] + list(range(1, 31)))
        for A in range(100, (MAX_A_VAL + 1000)/10, 100):
            print('A={}'.format(A))
            row = [A]
            for coin_set in coin_sets[:30]:
                start_time = time.time()
