...
struct Element
{
	void *elem;		/* value to store in array */
	long count;		/* count when element was added */
};

struct DynArr
{
	struct Element **data;	/* pointer to the data array */
	int size;		/* Number of elements in the array */
	int capacity;	/* capacity ofthe array */
	long counter;	/* total number of elements added ever */
};
...
void _initDynArr(DynArr *v, int capacity)
{
	assert(capacity > 0);
	assert(v!= 0);
	v->data = (struct Element **)malloc(sizeof(struct Element *) * capacity);
	assert(v->data != 0);
	v->size = 0;
	v->capacity = capacity;
	v->counter = 0;
...
{
	int i;
	struct Element **oldData = v->data;
	int oldSize = v->size;
	int oldCount = v->counter;
...
int containsDynArr(DynArr *v, void *val, comparator compare)
{
	...
	for (i = 0; i < sizeDynArr(v); i++)
      if ((*compare)(v->data[i]->elem, val) == 0)
         return 1;
      return 0;
...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
	...
	/* return index with smaller value, using count as tiebreaker */
	if (comp < 0 || (comp == 0 && heap->data[i]->count < heap->data[j]->count))
		return i;
	else
		return j;
}
...
*/
void addHeap(DynArr *heap, void *val, comparator compare)
{
	/* test preconditions */
	assert(heap != 0);
	...
	int parentPos;
	int comp; /* stores comparison return value */
	...
	while (pos > 0) {
		parentPos = (pos - 1) / 2;
		comp = (*compare)(getDynArr(heap, pos), getDynArr(heap, parentPos));
		
		/* check count when added as tiebreaker if priority is same */
		if ( comp < 0 
	 		 || (comp == 0 && heap->data[pos]->count < heap->data[parentPos]->count) ) {
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
	...
	if (right < max) {
		minPos = _smallerIndexHeap(heap, left, right, compare);
	} else if (left < max) {
		minPos = left;
	} else {
	...
	int comp = (*compare)(getDynArr(heap, minPos), getDynArr(heap, pos)); 
	if (comp < 0 || (comp == 0 && heap->data[minPos]->count < heap->data[pos]->count)) {
		swapDynArr(heap, minPos, pos);
		_adjustHeap(heap, max, minPos, compare);
	}
}
...
void _buildHeap(DynArr *heap, comparator compare)
{
	...
	while (startFrom >= 0) {
		_adjustHeap(heap, size, startFrom--, compare);
	}
